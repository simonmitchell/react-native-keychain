...
RCT_EXPORT_METHOD(getGenericPasswordForService:(NSString*)service callback:(RCTResponseSenderBlock)callback){
    ...
    NSDictionary *found = [self dictionaryForService:service error:&foundError];
    if (!found) {
        if (foundError) {
            return callback(@[makeError(foundError)]);
        } else {
...
RCT_EXPORT_METHOD(setObjectForService:(NSString *)service withObject:(NSDictionary *)object callback:(RCTResponseSenderBlock)callback) {
    ...
    dict = [NSDictionary dictionaryWithObjectsAndKeys:(__bridge id)(kSecClassGenericPassword), kSecClass, service, kSecAttrService, data, kSecValueData, service, kSecAttrAccount, nil];
...
RCT_EXPORT_METHOD(getObjectForService:(NSString *)service callback:(RCTResponseSenderBlock)callback) {
    ...
    // Found
    NSDictionary *object = [NSKeyedUnarchiver unarchiveObjectWithData:[found objectForKey:(__bridge id)(kSecValueData)]];
    
    callback(@[[NSNull null], object]);
}

RCT_EXPORT_METHOD(resetObjectForService:(NSString*)service callback:(RCTResponseSenderBlock)callback){
...
- (NSDictionary *)dictionaryForService:(NSString *)service error:(NSError **)error {
